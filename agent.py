"""
泰小虎智能健康导购助手 - 核心智能体模块 V3
基于 Prompt 完整文档 V1/V2 实现，修复多轮对话问题
V3: 集成阿里云百炼LLM，增强意图识别、健康建议生成、RAG知识库检索
...
# LLM服务配置 - 可通过环境变量控制是否启用
ENABLE_LLM = os.environ.get("ENABLE_LLM", "true").lower() == "true"

# 尝试导入LLM服务
try:
    from llm_service import get_llm_service
    llm_service = get_llm_service() if ENABLE_LLM else None
    print(f"[LLM] 服务已{'启用' if ENABLE_LLM else '禁用'}")
except ImportError:
    llm_service = None
    print("[LLM] 导入失败，使用规则引擎")
...
PRODUCT_DATABASE = [
    {
        "name": "鱼油软胶囊",
        "category": "心脑血管",
        "efficacy": "富含Omega-3不饱和脂肪酸，有助于维护心脑血管健康",
        "audience": "中老年人、关注心脑血管健康的人群",
        "usage": "每日2次，每次1粒，饭后服用",
        "symptoms": ["血压偏高", "血脂", "心脑血管", "心脏", "血管", "血液", "头晕", "头痛", "高血压", "心血管"],
...
class TaiXiaoHuAgent:
    ...
    def _get_knowledge_fallback(self, query: str) -> Optional[str]:
        """从知识库获取信息，用于兜底回复 - V3增强版，支持LLM RAG"""
        # 检查知识库
        ...
        # ---- V3扩展：从上传的知识库中搜索，使用LLM RAG ----
        try:
            from admin_service import knowledge_service
            results = knowledge_service.search_chunks(query, top_k=3)
            if results:
                # 记录调用统计
                for chunk in results:
                    knowledge_service.record_access(chunk.get('id'))
                
                # 如果有LLM服务，使用RAG生成回答
                if llm_service:
                    try:
                        rag_response = llm_service.rag_query(query, results)
                        if rag_response:
                            return rag_response
                    except Exception as e:
                        print(f"[LLM RAG] 生成失败: {e}")
    ...
    def _generate_symptom_summary(self) -> Dict:
        ...
        # 使用LLM或规则引擎生成健康建议
        health_advice = self._generate_health_advice_llm(main, duration, accompany, history)
    ...
    def _generate_health_advice_llm(self, main_symptom: str, duration: str, accompany: str, history: str) -> str:
        """使用LLM生成健康建议，如果不可用则回退到规则引擎"""
        # 构建症状信息
        symptom_info = {
        ...
        if hasattr(self, 'user_profile') and self.user_profile:
            user_profile = {
                ...
                "chronic_diseases": self.user_profile.chronic_diseases
            }
        
        # 尝试使用LLM生成建议
        if llm_service:
            try:
                llm_advice = llm_service.generate_health_advice(symptom_info, user_profile)
                if llm_advice:
                    return llm_advice
            except Exception as e:
                print(f"[LLM] 健康建议生成失败: {e}")
        
        # 回退到规则引擎
        all_symptoms = f"{main_symptom} {accompany}"
        return self._get_targeted_health_advice(all_symptoms)

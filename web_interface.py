...
def admin_knowledge_add():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': '切片添加成功', 'chunk': chunk})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def admin_knowledge_pull():
    ...
    try:
        ...
        return jsonify({'success': False, 'error': f'拉取失败: {str(e)}'}), 500
...
def admin_llm_get_config():
    """获取LLM配置"""
    from adapters.llm_adapter import llm_adapter
    return jsonify({'success': True, 'config': llm_adapter.get_safe_config()})


@app.route('/admin/llm/config', methods=['POST'])
def admin_llm_save_config():
    """保存LLM配置"""
